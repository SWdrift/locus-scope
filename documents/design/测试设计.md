...
测试证明 Core semantics、Package 发布与安装，以及 CLI 的可观察行为、易错点和复杂逻辑，不按源码函数逐一覆盖。
...
- **Package**（`internal/packages/*_test.go`）：验证 artifact 构建与校验、发布输入边界、lock、cache、物化和 resolution；覆盖跨包或持久化边界。
...
| 情况 | 用例 | Fixture |
| --- | --- | --- |
| 多 Scope 组合的 Entity、Relation、Group 和跨 Scope 解析 | `TestLoadScopeComposition` | `scope-composition` |
| Definition 专用后缀、普通目录递归、`.locusignore`、生成目录和嵌套 Scope 剪枝 | `TestDefinitionDiscoveryRecursesHonorsIgnoreAndStopsAtNestedScope` | `discovery` |
...
| Manifest、Entity、Import、Projection、Relation、重复 ID 和 `.locusignore` 的错误上下文 | `TestValidationDiagnostics` | `duplicate-expanded`、`missing-relation`、`validation/*` |
...
| 发布 layer 的确定性以及生成状态排除 | `TestBuildPackageLayerIsDeterministicAndExcludesGeneratedState` |
| 发布拒绝非可移植或逃出 Package root 的本地 Import | `TestValidatePackageSourceTreeRejectsNonPortableAndEscapingImports` |
| 发布拒绝 digest target | `TestPublishRejectsDigestTarget` |
| Artifact config 必须是 OCI empty JSON descriptor | `TestValidateCachedPackageRejectsNonEmptyConfig` |
...
| `publish` 参数约束和帮助输出 | `TestRunRejectsInvalidPublishInvocation`、`TestHelpIncludesPublishUsage` | 无 |
| 两个 CLI 的 `version`、`--version` 和 JSON 版本输出无需 Scope | 两个 `cmd` package 的 `TestRunShowsVersionWithoutScope` | 无 |
| 进程内 Distribution：缺省 Scope 向上发现、首次发布、幂等重发、同 tag 更新、首次安装、lock 复用、Frozen、离线查询和跨 Package ownership | `TestPackageCLIClosure` | `package` |
| 外部 Zot 上执行相同发布、更新和安装闭包 | `TestZotPackageCLIClosure` | `package`；设置 `LOCUS_TEST_REGISTRY` 后运行 |
