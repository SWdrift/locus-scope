...
- [公共协议](protocol/README.md)：汇总用户和工具直接依赖的稳定契约。
  ...
  - [CLI](protocol/CLI.md)：定义 `locus-scope`、`locus-scope-node` 和 `locus-pkg` 的指令、参数与输出约定。
- [Scope 设计](Scope设计.md)：以基本 Scope 装配和显式 Group 为例，定义 root 选择、递归 Definition document 发现、目录与嵌套 Scope 边界、Workspace 装配和诊断。
